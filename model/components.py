...
class EV(ap.Agent):
    ...
    def average_memory(self):
        ...
        '''
        self.average_price_memory = [
            round(np.mean(self.price_memory[i]), 2) for i in range(len(self.price_memory))]
    ...
    def choose_cheapest_hours(self, starting_time, ending_time, charge_needed):
        ...
        if starting_time % 24 < ending_time % 24:
            ...
            total_time_window = self.average_price_memory[starting_time %
                                                          24:ending_time % 24]
        else:
        ...
        if hours_needed > (abs(ending_time-starting_time)):
            print(
                'total time is insufficient to charge to full. Charging commencing immediately')
            return starting_time

        timewindow_copy = total_time_window.copy()
        timewindow_copy.sort()
        cheapest_values = timewindow_copy[:hours_needed]
        cheapest_starting_hours = [total_time_window.index(
            i) + starting_time for i in cheapest_values]

        print('the cheapest hour to start are hours {} with a total value of {}'.format(
            cheapest_starting_hours, cheapest_values))
